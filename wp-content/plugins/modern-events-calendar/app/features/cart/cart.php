...
<div class="mec-wrap mec-cart">
    ...
    <div class="mec-cart-empty-wrapper <?php echo (count($cart) ? 'mec-util-hidden' : ''); ?>">
        ...
        <?php if($archive_url): ?>
        <div>
            <a class="mec-bg-color button" href="<?php echo esc_url($archive_url); ?>"><?php esc_html_e('Go to events page', 'mec'); ?></a>
        </div>
        <?php endif; ?>
    </div>
    ...
    <table id="mec_cart_transactions_table">
        <thead>
            <tr>
                <th></th>
                <th><?php esc_html_e('Transaction ID', 'mec'); ?></th>
                <th><?php esc_html_e('Event', 'mec'); ?></th>
                <th><?php esc_html_e('Tickets', 'mec'); ?></th>
                <th><?php esc_html_e('Dates', 'mec'); ?></th>
                <th><?php esc_html_e('Payable', 'mec'); ?></th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr id="mec_cart_transactions_<?php echo esc_attr($transaction_id); ?>">
                ...
                <td>
                    ...
                    <?php echo ($TO->get_coupon() ? '<br><code class="mec-cart-coupon-code" title="'.esc_attr__('Applied Coupon', 'mec').'">'.esc_html($TO->get_coupon()).'</code>' : ''); ?>
                </td>
    ...
    <div class="mec-cart-coupon-checkout-action">
        ...
        <div class="mec-cart-checkout-button">
            <a class="mec-cart-checkout-link mec-bg-color button" href="<?php echo esc_url(get_permalink($this->settings['checkout_page'])); ?>"><?php esc_html_e('Proceed to checkout', 'mec'); ?></a>
        </div>
        <?php endif; ?>
    </div>
    <?php endif; ?>
