...
<div class="wrap" id="mec-wrap">
    ...
    <h2 class="nav-tab-wrapper">
        ...
        <a href="<?php echo esc_url($this->main->add_qs_var('tab', 'MEC-import')); ?>" class="nav-tab nav-tab-active"><?php echo esc_html__('Import', 'mec'); ?></a>
    ...
    <div class="mec-container">
        <div class="import-content w-clearfix extra">
            <h3><?php esc_html_e('Import MEC XML Feed', 'mec'); ?></h3>
            <form id="mec_import_xml_form" action="<?php echo esc_url($this->main->get_full_url()); ?>" method="POST" enctype="multipart/form-data">
                <div class="mec-form-row">
                    <p><?php echo sprintf(esc_html__("You can import %s events from another website to this website. You just need an XML feed of the events that can be exported from source website!", 'mec'), '<strong>'.esc_html__('Modern Events Calendar', 'mec').'</strong>'); ?></p>
                </div>
                <div class="mec-form-row">
                    ...
                    <button class="button button-primary mec-button-primary mec-btn-2"><?php esc_html_e('Upload & Import', 'mec'); ?></button>
                </div>
            </form>
            ...
            <br><h3><?php esc_html_e('Import .ics File', 'mec'); ?></h3>
            <?php if($this->getPRO()): ?>
            <form id="mec_import_ics_form" action="<?php echo esc_url($this->main->get_full_url()); ?>" method="POST" enctype="multipart/form-data">
                <div class="mec-form-row">
                    <p><?php echo sprintf(esc_html__("ICS format supports by many different service providers like Facebook. Apple Calendar etc. You can import your ics file into the %s using this form.", 'mec'), '<strong>'.esc_html__('Modern Events Calendar', 'mec').'</strong>'); ?></p>
                </div>
                <div class="mec-form-row">
            ...
            <br><h3><?php esc_html_e('Import Booking CSV File', 'mec'); ?></h3>
            <?php if($this->getPRO()): ?>
            <form id="mec_import_csv_booking_form" action="<?php echo esc_url($this->main->get_full_url()); ?>" method="POST" enctype="multipart/form-data">
                <div class="mec-form-row">
                    <p><?php echo sprintf(esc_html__("You can export bookings from %s using the booking menu in source website. You need a CSV export and then you're able to simply import it using this form in to your target website.", 'mec'), '<strong>'.esc_html__('Modern Events Calendar', 'mec').'</strong>'); ?></p>
                    <p><?php echo sprintf(esc_html__("If you want to create the booking file yourself you can check %s as a sample file.", 'mec'), '<a href="'.esc_url($this->main->asset('files/sample.csv')).'">sample.csv</a>'); ?></p>
                    <p style="color: red;"><?php echo esc_html__("Please note that you should create (or imports) events and tickets before importing the bookings otherwise booking won't import due to lack of data.", 'mec'); ?></p>
                </div>
                <div class="mec-form-row">
