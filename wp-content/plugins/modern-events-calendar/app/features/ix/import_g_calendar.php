...
<div class="wrap" id="mec-wrap">
    ...
    <h2 class="nav-tab-wrapper">
        <a href="<?php echo esc_url($this->main->remove_qs_var('tab')); ?>" class="nav-tab nav-tab-active"><?php echo esc_html__('Google Cal. Import', 'mec'); ?></a>
        ...
        <div class="import-content w-clearfix extra">
            <div class="mec-google-import">
                <form id="mec_google_import_form" action="<?php echo esc_url($this->main->get_full_url()); ?>" method="POST">
                    <h3><?php esc_html_e('Import from Google Calendar', 'mec'); ?></h3>
                    <p class="description"><?php esc_html_e('This will import all of your Google calendar events into MEC.', 'mec'); ?></p>
                    <div class="mec-form-row">
                        <label class="mec-col-3" for="mec_ix_google_import_api_key"><?php esc_html_e('Google API Key', 'mec'); ?></label>
                        <div class="mec-col-4">
                    ...
                    <div class="mec-form-row">
                        <label class="mec-col-3" for="mec_ix_google_import_calendar_id"><?php esc_html_e('Calendar ID', 'mec'); ?></label>
                        <div class="mec-col-4">
                    ...
                    <div class="mec-form-row">
                        <label class="mec-col-3" for="mec_ix_google_import_start_date"><?php esc_html_e('Start Date', 'mec'); ?></label>
                        <div class="mec-col-4">
                    ...
                    <div class="mec-form-row">
                        <label class="mec-col-3" for="mec_ix_google_import_end_date"><?php esc_html_e('End Date', 'mec'); ?></label>
                        <div class="mec-col-4">
                    ...
                    <div class="mec-options-fields">
                        ...
                        <button id="mec_ix_google_import_form_button" class="button button-primary mec-button-primary" type="submit"><?php esc_html_e('Start', 'mec'); ?></button>
                    </div>
                </form>
                ...
                <div class="mec-ix-google-import-started">
                    ...
                    <form id="mec_google_import_do_form" action="<?php echo esc_url($this->main->get_full_url()); ?>" method="POST">
                        <div class="mec-xi-google-import-events mec-options-fields">
                            <h4><?php esc_html_e('Google Calendar Events', 'mec'); ?></h4>
                            <div class="mec-success"><?php echo sprintf(esc_html__('We found %s events for %s calendar. Please select your desired events to import.', 'mec'), '<strong>'.esc_html($this->response['data']['count']).'</strong>', '<strong>'.esc_html($this->response['data']['title']).'</strong>'); ?></div>
                            ...
                            <ul id="mec_import_g_calendar_events">
                                ...
                                <?php
                                    $date_start = new DateTime((
                                        (isset($event['start']->date) && trim($event['start']->date))
                                        ? $event['start']->date
                                    ...
                                    $date_end = new DateTime((
                                        (isset($event['end']->date) && trim($event['end']->date))
                                        ? $event['end']->date
                                        : $event['end']->dateTime
                                    ));
                                ...
                                <li>
                                    <label>
                                        ...
                                        <span><?php echo sprintf(esc_html__('Event Title: %s Event Date: %s - %s', 'mec'), '<strong>'.esc_html($event['title']).'</strong>', '<strong title="'.esc_attr('First Date of Event', 'mec').'">'.($date_start->format('Y-m-d H:i:s')).'</strong>', '<strong title="'.esc_attr('First Date of Event', 'mec').'">'.($date_end->format('Y-m-d H:i:s')).'</strong>'); ?></span>
                                    </label>
                                </li>
                                <?php endforeach; ?>
                            </ul>
                        ...
                        <div class="mec-options-fields">
                            ...
                            <button id="mec_ix_google_import_do_form_button" class="button button-primary mec-button-primary" type="submit"><?php esc_html_e('Import', 'mec'); ?></button>
                        </div>
                    </form>
                    <?php endif; ?>
                </div>
                ...
                <div class="mec-ix-google-import-do">
                    ...
                    <div class="mec-error"><?php echo MEC_kses::element($this->response['error']); ?></div>
                    <?php else: ?>
                    <div class="mec-success"><?php echo sprintf(esc_html__('%s events successfully imported to your website from Google Calendar.', 'mec'), '<strong>'.count($this->response['data']).'</strong>'); ?></div>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
